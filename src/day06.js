...
export default function sumPairs(numbers, result) {
    for (let index = 0; index < numbers.length; index++) {
        ...
        // const internalNumber = numbers.filter((v, indexV) => indexV !== index);
        const internalNumber = numbers.slice(index + 1);
        for (const number2 of internalNumber) {
            if (number + number2 === result) {
                // console.log(numbers, internalNumber, index, number);
