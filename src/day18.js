...
export default function fixFiles(files) {
    let countFiles = {};
    let newFiles = [];
    // Armo un obj para seguir la cuenta de los archivos
    files.forEach(element => {
        countFiles[element] = countFiles[element] ? countFiles[element] : 0; // Fix Jest
        // countFiles[element] ??= 0;
    });

    // ReasignoNombre
    files.forEach(fileName => {
        const n = countFiles[fileName];
        countFiles[fileName]++;
        let newFileName = fileName + '(' + n + ')';
        if (!n) newFileName = fileName; // Para el archivo 0
