...
export default function decodeNumber(symbols) {
    ...
    symbols.split('').reverse().forEach(character => {
        let characterValue = value[character];
        characterValue = characterValue ? characterValue : NaN; // Fix Jest
        // characterValue ??= NaN;
        if (valueAnterior <= characterValue) {
            total += characterValue;
        } else {
            total -= characterValue;
        }

        valueAnterior = characterValue;
    });
    return total;
}
