...
export default function getCoins(change) {
    // ¡No olvides compartir tu solución en redes!t
    const coinsValues = [50, 20, 10, 5, 2, 1];
    const coinCount = [];
    let nextValue = change;
    coinsValues.forEach(coins => {
        const eachValue = ~~(nextValue / coins);
        nextValue = nextValue % coins;
        coinCount.unshift(eachValue);
    });
    return coinCount;
}