...
export default function countDecorations(bigTree) {
  return sumDecoration(bigTree);
}

function sumDecoration(tree) {
  let suma = 0;
  if (!tree) return 0;
  Object.entries(tree).forEach(([subTreeKey, subTree]) => {
    if (subTreeKey === 'value' || !subTree) { suma += subTree; return; }
    suma += subTree.value;
    suma += sumDecoration(subTree.left);
    suma += sumDecoration(subTree.right);
  });
  return suma;
}