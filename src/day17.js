...
export default function countPackages(carriers, carrierID) {
    const carriersObj = convert(carriers);
    const resultado = sumarPaquetes(carriersObj, [carrierID]);

    return resultado;
}

function sumarPaquetes(carriers, ids) {
    let suma = 0;
    ids.forEach(id => {
        const carrier = carriers.find(element => element.id === id);
        suma += carrier.paquetes;
        suma += sumarPaquetes(carriers, carrier.subordinados);
    });
    return suma;
}
